...
def jacobi(A, b, x0, TOL, maxIters, printer=False):
    ...
    if printer:
        ...
        print('------------------------------------------------')
        print('{:2d} | {: .4f} | {: .4f} | {: .4f} | {:7.4f} |'.format(0, x[0][0], x[1][0], x[2][0], residual))
...
#            s2 = np.dot(A[i,i+1:], x0[i+1:])
        ...
        residual = la.norm(b - np.matmul(A,x0))
        ...
        if printer:
            print('{:2d} | {: .4f} | {: .4f} | {: .4f} | {:7.4f} |'.format(k, x[0][0], x[1][0], x[2][0], residual ))
...
#            s2 = np.dot(A[i,i+1:], x0[i+1:]) 
        ...
        res = la.norm(r)
        if printer:
            print('{:2d} | {: .5f} | {: .5f} | {: .5f} | {: 2.5f} |'.format(k, x[0], x[1], x[2],res ))
